...
export default function QuadraEdit() {
  ...
  const loadQuadra = async () => {
    try {
      const data = await api.getQuadra(quadraId)
      if (data) {
        setQuadra(data)
        form.reset({
          name: data.name,
      ...
      console.error('Error loading quadra:', error)
  ...
  const onSubmit = async (values: FormValues) => {
    if (!quadraId || !canEditProjects) return

    setSaving(true)
    try {
      await api.updateQuadra(quadraId, {
        name: values.name,
        area: values.area,
      ...
      <div className="h-[calc(100vh-100px)] flex flex-col items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-gray-500">
            Você não tem permissão para editar quadras.
          </p>
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Editar Quadra</h1>
          <p className="text-muted-foreground">
            {quadra?.name || 'Carregando...'}
          </p>
        </div>
      </div>
